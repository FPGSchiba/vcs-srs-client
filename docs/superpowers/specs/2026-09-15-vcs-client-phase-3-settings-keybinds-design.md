...
**macOS Accessibility permission — mandatory manual pass (R12).** The
executable procedure lives in
`docs/superpowers/plans/2026-09-15-phase-3-manual-verification.md` **§10**,
which is the authority; it is not restated here. Duplicating it is what let
the two copies drift the first time — a sweep corrected R12 and this section
while §10 kept telling testers to reset and grant *Input Monitoring*, an
instruction that reproduces the very bug being verified.

Two constraints that govern the whole pass, recorded here because they are
spec-level facts rather than steps:

- **It cannot be automated.** A `go test` binary is never a trusted app, so
  both Accessibility and Input Monitoring read `false` for it and CI cannot
  tell the two services apart at all. `internal/hotkeys/permission_darwin_test.go`
  pins the API and the settings-pane anchor; everything behavioural needs a
  human and a real TCC.
- **It must run on an account that has never granted VCS Accessibility.** A
  machine already holding the grant also satisfies the Input Monitoring
  preflight, so every item passes vacuously — exactly how the wrong-service
  bug survived. Reset with `tccutil reset Accessibility`, or use a fresh user.

Nothing in this flow has yet been exercised against a real TCC. Until §10 has
been run, R12 is mitigated in code and unverified in practice.
